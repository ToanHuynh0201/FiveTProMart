...
import { salaryService, staffService } from "@/services";
	...
	const loadStaffSalaryDetail = async () => {
		...
		try {
			...
			if (response.success && response.data) {
				...
				// Fetch real name from staff API
				try {
					const staffInfo = await staffService.getStaffById(staffId);
					if (staffInfo.success && staffInfo.data) {
						setRealName(staffInfo.data.fullName);
					}
				} catch (err) {
					console.warn("Failed to fetch staff name:", err);
				}
			} else {
			...
			toast({
				title: "Lỗi",
				description: error?.message || "Nhân viên không tồn tại hoặc chưa có dữ liệu lương",
				status: "error",
				duration: 3000,
				isClosable: true,
			});
						...
						<VStack align="stretch" spacing={6}>
							...
							<Box bg="blue.50" p={4} borderRadius="md">
								...
								<Text fontSize="sm" color="gray.600">
									Kỳ lương: {data.range.fromDate} - {data.range.toDate}
								</Text>
							</Box>
